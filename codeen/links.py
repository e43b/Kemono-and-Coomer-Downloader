...
def get_total_posts(profile_url):
    ...
    if paginator_info and paginator_info.find('small'):
        ...
        # Count posts directly if paginator is not present
        total_posts = len(soup.find_all('article', {'class': 'post-card'}))

    return total_posts

# Function to get post links from a specific page
...
# Function to get author name, site domain, and platform
def get_author_platform_info(profile_url):
    ...
    # Get site domain and platform name
...
# Function to process command line arguments and determine pages to scrape
...
def scrape_fansly(profile_url, pages_arg):
    ...
    total_pages = (total_posts + 49) // 50  # To cover all posts, round up
    ...
    for page in pages_to_scrape:
        posts = get_posts_links(profile_url, page - 1)  # Adjust for page index
        data['pages'].append({
            'page_number': page,
            'posts': posts
        })
    ...
    # Create the JSON filename dynamically
...
def main():
    parser = argparse.ArgumentParser(
        description='Scrape posts links from a profile.',
        epilog='''
        Usage examples:
          python links.py https://coomer.su/fansly/user/44 all
            python links.py https://kemono.su/fanbox/user/20 "1 to 5"
            python links.py https://kemono.su/patreon/user/11 "-8 to -10, -25"
            python links.py https://coomer.su/fansly/user/2 "1, 6, 9"
        ''',
